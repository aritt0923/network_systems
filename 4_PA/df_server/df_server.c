...
int main(int argc, char *argv[])
{
    if (argc != 3)
    {
        printf("Usage: ./dfs <directory> <port #>\n");
        return 1;
    }
    ...
    hash_table *file_cabinet = calloc_wrap(1, sizeof(hash_table));
    int buckets = 100;
    create_hash_table(file_cabinet, buckets);
    memcpy(file_cabinet->dfs_dir, argv[1], strlen(argv[1]));

    thread_args args; // struct passed to requester threads
    args.cache = file_cabinet;
...
void *connection_handler(void *vargs)
{
    ...
    while (1)
    {
        ...
        int res = 0;
        if ((res = handle_cmd_dfs(args->cache, client_message, client_sock, args->socket_sem)) != 0)
        {
